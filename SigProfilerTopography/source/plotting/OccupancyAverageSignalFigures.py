...
def plotSignatureBasedAverageOccupancyFigureWithSimulations(sample,
    ...
    if ((realAverage is not None) and (pd.notna(realAverage).any(axis=0)) and (np.any(realAverage))):
        ...
        # delete starts
        if signature == 'SBS4':
            np.save(os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_real_nucleosome_occupancy'), realAverage)
            np.save(os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_means_nucleosome_occupancy'), np.array(simulationsSignatureBasedMeans))
            np.save(os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_lows_nucleosome_occupancy'), np.array(simulationsSignatureBasedLows))
            np.save(os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_highs_nucleosome_occupancy'), np.array(simulationsSignatureBasedHighs))

            realAverage_loaded = np.load(
                os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_real_nucleosome_occupancy.npy'))
            simulationsSignatureBasedMeans_loaded = np.load(
                os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_means_nucleosome_occupancy.npy'))
            simulationsSignatureBasedLows_loaded = np.load(
                os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_lows_nucleosome_occupancy.npy'))
            simulationsSignatureBasedHighs_loaded = np.load(
                os.path.join(outputDir, jobname, DATA, occupancy_type, 'SBS4_simulations_highs_nucleosome_occupancy.npy'))

            print(realAverage == realAverage_loaded)
            print(np.array(simulationsSignatureBasedMeans) == simulationsSignatureBasedMeans_loaded)
            print(np.array(simulationsSignatureBasedLows) == simulationsSignatureBasedLows_loaded)
            print(np.array(simulationsSignatureBasedHighs) == simulationsSignatureBasedHighs_loaded)

            print(np.all(realAverage == realAverage_loaded))
            print(np.all(np.array(simulationsSignatureBasedMeans) == simulationsSignatureBasedMeans_loaded))
            print(np.all(np.array(simulationsSignatureBasedLows) == simulationsSignatureBasedLows_loaded))
            print(np.all(np.array(simulationsSignatureBasedHighs) == simulationsSignatureBasedHighs_loaded))

            print('realAverage:',
                  'np.shape(realAverage):', np.shape(realAverage),
                  'realAverage.ndim:', realAverage.ndim)

            print('simulationsSignatureBasedMeans_loaded:',
                  'np.shape(simulationsSignatureBasedMeans_loaded):', np.shape(simulationsSignatureBasedMeans_loaded),
                  'simulationsSignatureBasedMeans_loaded.ndim:', simulationsSignatureBasedMeans_loaded.ndim)

            print('np.array(simulationsSignatureBasedMeans):',
                  'np.shape(np.array(simulationsSignatureBasedMeans)):', np.shape(np.array(simulationsSignatureBasedMeans)),
                  'np.array(simulationsSignatureBasedMeans).ndim:', np.array(simulationsSignatureBasedMeans).ndim)

            print('simulationsSignatureBasedLows_loaded:',
                  'np.shape(simulationsSignatureBasedLows_loaded):', np.shape(simulationsSignatureBasedLows_loaded),
                  'simulationsSignatureBasedLows_loaded.ndim:', simulationsSignatureBasedLows_loaded.ndim)

            print('np.array(simulationsSignatureBasedLows):',
                  'np.shape(np.array(simulationsSignatureBasedLows)):', np.shape(np.array(simulationsSignatureBasedLows)),
                  'np.array(simulationsSignatureBasedLows).ndim:', np.array(simulationsSignatureBasedLows).ndim)

            print('simulationsSignatureBasedHighs_loaded:',
                  'np.shape(simulationsSignatureBasedHighs_loaded):', np.shape(simulationsSignatureBasedHighs_loaded),
                  'simulationsSignatureBasedHighs_loaded.ndim:', simulationsSignatureBasedHighs_loaded.ndim)

            print('np.array(simulationsSignatureBasedHighs):',
                  'np.shape(np.array(simulationsSignatureBasedHighs)):', np.shape(np.array(simulationsSignatureBasedHighs)),
                  'np.array(simulationsSignatureBasedHighs).ndim:', np.array(simulationsSignatureBasedHighs).ndim)

            print(signature, 'For NSLC SBS4 positive samples only, realAverage:',
                  realAverage,
                  type(realAverage),
                  len(realAverage))
            ...
            print(signature, 'For NSLC SBS4 positive samples only, realAverage:',
                  realAverage.tolist(),
                  type(realAverage.tolist()),
                  len(realAverage.tolist()))

            print(signature, 'For NSLC SBS4 positive samples only, simulationsSignatureBasedMeans:',
                  simulationsSignatureBasedMeans,
                  type(simulationsSignatureBasedMeans),
                  len(simulationsSignatureBasedMeans))
            print(signature, 'For NSLC SBS4 positive samples only, simulationsSignatureBasedLows:',
                  simulationsSignatureBasedLows,
                  type(simulationsSignatureBasedLows),
                  len(simulationsSignatureBasedLows))
            print(signature, 'For NSLC SBS4 positive samples only, simulationsSignatureBasedHighs:',
                  simulationsSignatureBasedHighs,
                  type(simulationsSignatureBasedHighs),
                  len(simulationsSignatureBasedHighs))
        # delete ends
