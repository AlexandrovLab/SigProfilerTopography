...
def readWig_with_fixedStep_variableStep_writeChrBasedSignalArrays(outputDir, jobname, genome, wig_file_path, occupancy_type, quantileValue, remove_outliers, log_file):
    chromSizesDict = getChromSizesDict(genome)
    ...
    if os.path.exists(wig_file_path):
        ...
        print('Mean Signal: %f' % mean_signal, file=log_out)
        print('Std Signal: %f' % std_signal, file=log_out)
        ...
        if ((remove_outliers==True) and (quantileValue < 1.0)):
            ...
            print('Signal greater than %f is removed for quantile: %f' % (q,quantileValue), file=log_out)
            print('before outlier removal number of rows: %d' % (wigfile_interval_version_df.shape[0]), file=log_out)
            file_df = wigfile_interval_version_df[wigfile_interval_version_df[SIGNAL] < q]
            print('after outlier removal number of rows: %d' % (file_df.shape[0]), file=log_out)
        else:
...
def readWig_write_derived_from_bedgraph_using_pool_read_all(outputDir, jobname, genome, library_file_with_path, occupancy_type, remove_outliers, verbose, quantileValue, log_file):
    chromSizesDict = getChromSizesDict(genome)
    possible_chrom_list = list(chromSizesDict.keys())
    log_out = open(log_file, 'a')
    if verbose: print('\tVerbose possible_chrom_list:%s' % (possible_chrom_list), file=log_out)
    ...
    if os.path.exists(library_file_with_path):
        ...
        if verbose: print('\tVerbose After data_df[CHROM].unique():%s' % (data_df[CHROM].unique()), file=log_out)
        ...
        print('\nfile_df.shape:(%d,%d)' % (data_df.shape[0], data_df.shape[1]), file=log_out)
        print('Max Signal: %f' % max_signal, file=log_out)
        print('Min Signal: %f' % min_signal, file=log_out)
        print('Mean Signal skipna=True: %f' % mean_skipna_True_signal, file=log_out)
        print('Mean Signal skipna=False: %f' % mean_skipna_False_signal, file=log_out)
        print('Std Signal: %f' % std_signal, file=log_out)

        if verbose: print('\tVerbose %s Worker pid %s type(data):%s' %(occupancy_type, str(os.getpid()),type(data_df)), file=log_out)
        if verbose: print('\tVerbose %s Worker pid %s sys.getsizeof(data):%s' %(occupancy_type, str(os.getpid()),sys.getsizeof(data_df)), file=log_out)
        if verbose: print('\tVerbose ################################', file=log_out)

        if (remove_outliers and (quantileValue < 1.0)):
            # remove the outliers
            q = data_df[SIGNAL].quantile(quantileValue)
            print('\tq:%f' % q, file=log_out)
            print('\tbefore outlier elimination number of rows: %d' % (data_df.shape[0]), file=log_out)
            data_df = data_df[data_df[SIGNAL] < q]
            print('\tafter outlier elimination number of rows: %d' % (data_df.shape[0]), file=log_out)
        ...
        for chrname,chrBased_data_df in data_df_grouped:
            if chrname in possible_chrom_list:
                ...
                if verbose: print('\tVerbose JOB chrName:%s chrBased_data_df(%d,%d)' % (chrname, chrBased_data_df.shape[0], chrBased_data_df.shape[1]), file=log_out)

        # wait for all jobs to finish
        for job in jobs:
            if verbose: print('\tVerbose %s Worker pid %s job.get():%s ' % (occupancy_type, str(os.getpid()), job.get()), file=log_out)
...
def readFileInBEDFormat(file_with_path, discard_signal, log_file):
    file_df = None
    log_out = open(log_file,'a')
    ...
    if os.path.exists(file_with_path):
        ...
        if (ncols == 3 and discard_signal):
            ...
            if (ncols == 10):
                print('--- ENCODE narrowpeak BED6+4', file=log_out)
            elif (ncols == 9):
        ...
        if SIGNAL in file_df.columns.values:
            ...
            print('--- Mean Signal skipna=False: %f' %mean_skipna_False_signal, file=log_out)
            print('--- Std Signal: %f' %std_signal, file=log_out)
            print('--- file_df[SIGNAL].isnull().sum():%d' %file_df[SIGNAL].isnull().sum(), file=log_out)
            log_out.close()
...
def readBEDandWriteChromBasedSignalArrays(outputDir, jobname, genome, BEDFileWithPath, occupancy_type, quantileValue, remove_outliers, log_file):
    chromSizesDict = getChromSizesDict(genome)
    numofProcesses = multiprocessing.cpu_count()
    ...
    if os.path.exists(BEDFileWithPath):
        ...
        if (bedfile_extention.lower()=='.bed' or bedfile_extention.lower()=='.np' or bedfile_extention.lower()=='.narrowpeak'):
            ...
            if ((remove_outliers==True) and (quantileValue < 1.0)):
                ...
                log_out = open(log_file, 'a')
                print('Signal greater than %f is removed for quantile: %f' % (q,quantileValue), file=log_out)
                print('before outlier removal number of rows: %d' % (file_df.shape[0]), file=log_out)
